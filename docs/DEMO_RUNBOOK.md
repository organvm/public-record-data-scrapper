...
| `PLAID_CLIENT_ID`, `PLAID_SECRET` (`PLAID_ENV`)                               | `server/integrations/plaid/client.ts`                                                                      | Real bank-data extraction for `POST /api/prospects/:id/underwrite` (Plaid `accessToken`) — without it underwriting fails closed with `MISSING_UNDERWRITING_INPUTS`.                                                                                     | Plaid production keys                   |
| `PLAID_WEBHOOK_SECRET`                                                        | `server/config/index.ts` (prod validation)                                                                 | Verified Plaid webhooks; required in production.                                                                                                                                                                                                        | Plaid webhook secret                    |
...
| `INBOUND_PARSE_TOKEN`                                                         | _(not yet referenced in code)_                                                                             | Planned token for an inbound-email parse webhook. As of this phase the wired inbound-email verification env is `SENDGRID_WEBHOOK_VERIFICATION_KEY` (above); `INBOUND_PARSE_TOKEN` is a Phase-2 plan name with no consumer yet. Do not promise it works. | SendGrid webhook secret                 |

Production-only guard: when `NODE_ENV=production`, `validateConfig` (`server/config/index.ts`) hard-fails
startup unless `STRIPE_WEBHOOK_SECRET`, `TWILIO_AUTH_TOKEN`, `PLAID_WEBHOOK_SECRET` and
`SENDGRID_WEBHOOK_VERIFICATION_KEY` are present. The local demo runs as `development`, so these are
optional — their absence simply keeps the corresponding sends/webhooks fail-closed.
